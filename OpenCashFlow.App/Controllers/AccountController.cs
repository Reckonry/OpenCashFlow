...
namespace OpenCashFlow.App.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            // Se fallisce, passa comunque la view (Model nullo). La view gestirà gracefully i campi mancanti
        ...
        public async Task<IActionResult> EditAccount()
        {
            ...
            {
                ViewBag.ErrorMessage = "Impossibile caricare i dati del profilo";
        ...
        public async Task<IActionResult> EditAccount(Employee_MyProfile_Update_DTO model)
        {
            ...
            if (!result.Success)
            {
                ViewBag.ErrorMessage = result.Message ?? "Errore durante il salvataggio";
                return View("Edit_Account", model);
            }

            // Regenera il token JWT con i claims aggiornati per aggiornare la navbar senza logout
            ...
            ViewBag.SuccessMessage = "Profilo aggiornato";

            // Ricarica i dati aggiornati per la view
            var userId = GetCurrentUserId();
            if (userId != null)
            {
                var detail = await _employeeAPIService.GetEmployeeByIDAsync(userId.Value);
        ...
        public async Task<IActionResult> ChangePassword(PasswordVM vm)
        {
            ...
            {
                ViewBag.ErrorMessage = "Le password non coincidono";
            ...
            if (!resp.Success)
            {
                ViewBag.ErrorMessage = resp.Message ?? "Impossibile cambiare password";
                return await LoadEditAccountView(userId.Value);
            }

            ViewBag.SuccessMessage = "Password aggiornata";
            return await LoadEditAccountView(userId.Value);
        }
