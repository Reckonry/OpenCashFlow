...
namespace OpenCashFlow.API.Services
{
    public partial class AuthenticationService : IAuthenticationService
    {
        ...
        public async Task<AuthResult> Authenticate(string username, string password, CancellationToken cancellationToken)
        {
            ...
            var user = await _authenticationRepository.GetUserByUsernameAndPasswordAsync(username, password, cancellationToken); // Recupera l'utente dal repository
            if (user == null) return new AuthResult() { Success = false, ErrorType = AuthErrorType.InvalidCredentials }; // Verifica se l'utente esiste e la password è corretta
            ...
            var claims = new List<Claim>
            {
                ...
                new("Timezone", "Europe/Rome"), //todo: aggiungere timezone corretto
        ...
        public async Task<ApiResponse<string?>> AuthenticateFastAsync(HttpContext httpContext, string pin, string FLCookieValue, CancellationToken cancellationToken)
        {
            ...
            // Firma non valida o cookie corrotto → rimuovo il cookie ed esco
            ...
            // Aggiungo le claim dei ruoli REALI dell'utente (no hardcode)
            // Se la navigazione non è caricata, leggo da DB per sicurezza
            try
            {
            ...
            {
                // In caso di problemi di caricamento, non fallire il login fast
            }

            // Recupero eventuali claim personalizzate dell'utente
