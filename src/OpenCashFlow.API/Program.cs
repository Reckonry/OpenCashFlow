...
// // Enable Sentry only if DSN is present and valid (avoid startup crash in local/dev)
...
//     try
//     {
//         // Validate DSN format (throws if invalid)
//         _ = new Dsn(sentryDsn);
//         sentryEnabled = true;
...
//             // lascia pure i default, poi semmai setti sample rate / env ecc.
//         });
//     }
//     catch (Exception ex)
//     {
...
// builder.Logging.SetMinimumLevel(LogLevel.Warning); // Set to Warning or Error
...
// {
//     // Global rule: limit all requests to 5 per second
//     options.AddFixedWindowLimiter("global", limiterOptions =>
//     {
//         limiterOptions.Window = TimeSpan.FromSeconds(1); // 1-second window
//         limiterOptions.PermitLimit = 5;                 // 5 requests per window
//         limiterOptions.QueueProcessingOrder = QueueProcessingOrder.OldestFirst; // Queue order
//         limiterOptions.QueueLimit = 2;                 // Allow 2 queued requests
//     });

//     // Rule for a specific endpoint
...
//         limiterOptions.PermitLimit = 20;                // 20 requests per window
//     });
// });
// #endregion
// #region Bearer Auth
...
//     options.DefaultApiVersion = new ApiVersion(1, 0);
//     options.AssumeDefaultVersionWhenUnspecified = true;
//     options.ReportApiVersions = true; // Send supported versions in the response header
// });
...
//         // Try to resolve the client (fails if the key is missing)
...
//         {
//             // Works with SqlServer/Postgres/SQLite file
...
// public partial class Program { } // DO NOT REMOVE: required for CI/CD tests with in-memory DB on the API
